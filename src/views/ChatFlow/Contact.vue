  ...
  <div class="pa-4" style="color: #adb5bd">
    ...
    <div>
      <div v-for="(item, index) in charList" :key="index">
        <div class="d-flex">
          <label style="font-size: 12px; color: #4eac6d" class="mr-5">{{
            item
          }}</label>
...
import ContactItem from "../../components/SideSection/Contact/ContactItem.vue";
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      this.contactList[char] = this.contactListCopy[char].filter((item) =>
        item.contact_name
          .toLowerCase()
