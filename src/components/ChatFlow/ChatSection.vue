<template>
  <div class="main">
    <div style="width: 100%">
      <div
        v-if="activeProfile != null"
        class="header d-flex justify-space-between"
      >
        <div class="d-flex my-auto">
          ...
          <v-avatar size="40px" class="my-auto">
            <img
              v-if="
                ...
                activeProfile.user.profile_image != 'Icon'
              "
          ...
          <div class="my-auto ml-3">
            <label>{{
              activeProfile != null ? activeProfile.contact_name : ""
            }}</label
            ><br />
        ...
        <div class="d-flex my-auto">
          ...
          <v-btn @click="drawer = !drawer" icon fab small dark class="menuOptions">
      ...
      >
        <div
          v-for="(item, index) in $store.state.chat.chat.messages"
          :key="index"
          style="width: 100%"
        >
    ...
    async loadNextPage() {
      ...
      if (el.scrollTop == 0) {
        try {
          ...
          let messages = await Chat.loadUserChats(
            this.activeProfile.contact_id,
            this.activeProfile.room_id,
          ...
          if (reversed.length > 0) {
            ...
            for (let i = 0; i < reversed.length; i++) {
              if (
                reversed[i].is_read == false &&
                reversed[i].to_user_id == this.$store.state.currentUser.id
              ) {
